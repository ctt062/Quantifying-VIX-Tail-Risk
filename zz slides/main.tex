...
\begin{enumerate}
    ...
    \item \textbf{Regime Dependence:} COVID showed 41\% higher shock rate than baseline.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Hawkes intensity can provide real-time ``shock alert'' signals.
    \end{itemize}
    \item \textbf{Option Pricing:}
    \begin{itemize}
        \item Leverage effect matters: adjust for skewed vol-of-vol.
        \item Shock clustering affects term structure of implied volatility.
    \end{itemize}
    \item \textbf{Portfolio Allocation:}
    \begin{itemize}
        \item Regime detection enables adaptive hedging strategies.
        \item 4-day Hawkes half-life suggests momentum in shock risk.
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Large spikes arrive at $\sim$9/year and exhibit \textbf{self-excitation} (Hawkes branching ratio 0.23).
    \item \textbf{COVID regime} showed significantly elevated shock rates (+41\%).
    \item Out-of-sample, \textbf{EWMA remains a tough benchmark} to beat for density forecasting.
    \item The reproducible pipeline (\texttt{runall.py}) with \textbf{23 unit tests} enables transparent research.
\end{itemize}
...
|   +-- shock_modeling.py      # HPP/NHPP/Hawkes
|   +-- forecast_evaluation.py # OOS evaluation
|   +-- visualization.py       # Plotting
+-- tests/
|   +-- test_models.py         # 23 unit tests
